...
/** Optional placement for helper-built geometry. Rotation is XYZ Euler, radians. */
...
/**
 * Box whose UVs are in metres, so tiling textures keep a constant real-world
 * scale on every face regardless of the box dimensions.
 */
...
/** Box defined by its min corner and size; handy for architectural layout. */
...
/** Merge geometries of mixed origin (indexed or not) into one draw call. */
...
export interface Opening {
  /** Centre along the wall, metres from the wall's left end seen from outside. */
  u: number;
  /** Bottom of the opening in world height. */
  y: number;
  w: number;
  h: number;
}
...
export interface WallOutline {
  width: number;
  /** Bottom of the cladding (top of foundation). */
  y0: number;
  /** Eave height. */
  y1: number;
  /** Optional gable: peak height and its position along the wall. */
  gable?: { peakY: number; peakU?: number };
}

/**
 * Flat wall with real openings. UVs equal local (u, worldY) in metres so lap
 * siding courses line up around every corner of the house.
 */
export function wallGeometry(outline: WallOutline, openings: Opening[]): THREE.BufferGeometry {
  ...
  for (const n of notches) {
    shape.lineTo(n.u - n.w / 2, y0);
    shape.lineTo(n.u - n.w / 2, n.y + n.h);
    shape.lineTo(n.u + n.w / 2, n.y + n.h);
    shape.lineTo(n.u + n.w / 2, y0);
  }
  shape.lineTo(width, y0);
  shape.lineTo(width, y1);
  ...
  for (const o of holes) {
    ...
    const r = o.u + o.w / 2;
    path.moveTo(l, o.y);
    path.lineTo(l, o.y + o.h);
    path.lineTo(r, o.y + o.h);
    path.lineTo(r, o.y);
    path.closePath();
